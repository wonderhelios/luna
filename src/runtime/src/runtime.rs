...
impl LunaRuntime {
    ...
    pub async fn run_with_event_hook<F>(&self, req: RunRequest, mut hook: F) -> Result<RunResponse>
    where
        F: FnMut(&RuntimeEvent) + Send,
    {
        ...
        struct HookedEvents<'a, F: FnMut(&RuntimeEvent)> {
            events: Vec<RuntimeEvent>,
            hook: &'a mut F,
        }

        impl<'a, F: FnMut(&RuntimeEvent)> EventSink for HookedEvents<'a, F> {
            fn emit(&mut self, event: &RuntimeEvent) {
                (self.hook)(event);
                self.events.push(event.clone());
            }
            fn snapshot(&self) -> Option<&[RuntimeEvent]> {
                Some(self.events.as_slice())
            }
        }

        let mut hooked = HookedEvents {
            events: Vec::new(),
            hook: &mut hook,
        };

        let result = self.run_impl_with_sink(req, &mut session_id_for_end, &mut hooked);
        let result = result.map(|mut resp| {
            resp.events = hooked.events;
            resp
        });
    ...
    pub fn run_impl(
        ...
        let mut resp = self.run_impl_with_sink(req, session_id_for_end, &mut events)?;
        resp.events = events;
        Ok(resp)
    }

    fn run_impl_with_sink(
        &self,
        req: RunRequest,
        session_id_for_end: &mut Option<String>,
        ...
        if let Some(cmd) = command::parse_slash_command(&user_input) {
            ...
            let current_session_id = match &session {
                ...
                SessionRef::New { title } => {
                    ...
                    let ev = RuntimeEvent::SessionCreated {
        ...
        let mut session = match session {
            ...
            SessionRef::Existing { session_id } => {
                ...
                let ev = RuntimeEvent::SessionLoaded {
                    session_id: session.id.clone(),
                };
                events.emit(&ev);
                session
        ...
        *session_id_for_end = Some(session_id.clone());
        ...
        events.emit(&RuntimeEvent::AssistantMessageAppended);
