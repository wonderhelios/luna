    ...
    fn find_identifier_occurrences(content: &str, name: &str, max: usize) -> Vec<TextRange> {
        ...
        while i + needle.len() <= bytes.len() {
            ...
            if bytes[i..].starts_with(needle) {
                ...
                if prev_ok && next_ok {
                    out.push(TextRange {
                        start: crate::core_local::Position {
                            byte: start_byte,
                            line: start_line,
                            column: start_col,
                        },
                        end: crate::core_local::Position {
                            byte: end_byte,
                            line: end_line,
                            column: end_col,
                        },
...
impl<P: RepoFileProvider> Navigator for TreeSitterNavigator<P> {
    ...
    fn goto_definition(
        ...
        for file in files {
            ...
            for idx in sg.graph.node_indices() {
                ...
                let Some(crate::scope_resolution::NodeKind::Def(d)) = sg.get_node(idx) else {
                    continue;
                };
    ...
    fn goto_definition_at(
        ...
        if let Some(crate::scope_resolution::NodeKind::Def(d)) = sg.get_node(node_idx) {
        ...
        let defs = sg
            ...
            .filter_map(|def_idx| match sg.get_node(def_idx) {
                Some(crate::scope_resolution::NodeKind::Def(d)) => Some(SymbolLocation {
                    rel_path: rel_path.to_path_buf(),
                    range: d.range,
                }),
                _ => None,
...
impl<P: RepoFileProvider> TreeSitterNavigator<P> {
    fn semantic_references_in_file(
        ...
        for idx in sg.graph.node_indices() {
            ...
            let Some(crate::scope_resolution::NodeKind::Ref(r)) = sg.get_node(idx) else {
                continue;
            };
